...
class TeamcityStatisticsLogger {
    ...
    private fun printPerChunkStatistics(moduleChunk: ModuleChunk, timeToCompileNs: Long) {
        val moduleNames = moduleChunk.getModules().map { it.getName() }
        val modulesKey = "${KotlinBuilder.KOTLIN_BUILDER_NAME} for modules ${moduleNames
                .joinToString(separator = ", ", prefix = "[", postfix = "]")} compilation time, ms"

        printStatisticMessage(modulesKey, timeToCompileNs.nanosToMillis().toString())
    }
