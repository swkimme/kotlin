...
public abstract class AbstractNavigateToLibrarySourceTest : KotlinCodeInsightTestCase() {
    ...
    protected fun doTest(path: String): Unit = doTestEx(path, withJS = false)

    protected fun doWithJSModuleTest(path: String): Unit = doTestEx(path, withJS = true)

    protected fun doTestEx(path: String, withJS: Boolean) {
        configureByFile(path)
        configureModule(getModule(), getProjectDescriptor())
        if (withJS) {
            val jsModule = this.createModule("js-module")
            configureModule(jsModule, JetStdJSProjectDescriptor.INSTANCE)
        }
    ...
    private fun collectInterestingNavigationElements(): Collection<PsiElement> =
        ContainerUtil.map<JetReference, PsiElement>(collectInterestingReferences()) {
